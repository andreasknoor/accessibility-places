...
**Header radius pill (mobile)** — `MobileLayout`'s header renders a `RadiusPresetPopover` (`components/filters/RadiusPresetPopover.tsx`) next to the settings icon, using the same `radiusKm`/`onRadiusChange` props already passed down for `ResultsList`'s header radius picker — so it's outside the tab-switched content (`ChatPanel` sits there too, "always visible" per its own comment) and stays in sync across all three tabs without new state. `onRadiusChange` is `undefined` during an active amenity search (`canShowResultsRadiusPicker` in `lib/search-ui.ts`), which the shared component renders as a plain non-interactive label — same fallback `ResultsList` always had. `RADIUS_PRESETS_KM` (1/2/5/10/25/50 km) is exported from the shared component now, not duplicated.
