...
- **accessibility.cloud** (`accessibility-cloud.ts`): A11yJSON-shaped records. Always uses `accessibilityPreset=at-least-partially-accessible-by-wheelchair`. **Records mirrored from Wheelmap are dropped entirely** (`infoPageUrl` host `wheelmap.org` → `toPlace()` returns `null`, v11.10) — a 2026-07 5-round data-quality audit (`docs/analysis/acloud-*-2026-07.md`) found these are 83% of A.Cloud's DACH volume, never richer than the OSM data the app's own `osm.ts` already fetches live (wheelmap.org is itself just an editing UI on OSM — the node ID is embedded in the URL), and 25% point at OSM nodes already deleted. Consequence: `wheelmapUrl` (see the `Place` field list below) is no longer set by any source — the field, `merge.ts`'s preservation of it, and its UI consumers are left in place (harmless if always `undefined`) rather than ripped out, since that was explicitly out of scope for the audit's two agreed fixes (this filter + the reliability-weight cut below). Genuinely A.Cloud-only records (local accessibility surveys with no OSM/Ginto counterpart, concentrated in rural AT/CH) have no `wheelmap.org` `infoPageUrl` and are unaffected.
...
**Category classification is EXACT-MATCH ONLY — never `.includes()`.** `accessibility-cloud.ts` (`FROM_ACLOUD`) and `reisen-fuer-alle.ts` (`FROM_RFA`) map source category strings via lookup tables keyed on the exact, normalised value; `acceslibre.ts` (`FROM_ACCESLIBRE`) already did, and `osm.ts`/`google-places.ts` use `===`/array membership. A substring chain silently mis-classified ~4.4% of all A.Cloud records — `"public_transport"` matched `"pub"` (transit stops became pubs), `"barber"` matched `"bar"` (hairdressers became bars) — and dropped a further ~4.4% that had a valid target (`"bread"` → bakery). Each table is the **union** of values observed live and the keys the previous implementation was written against: with exact matching, extra keys cost nothing (they can only match themselves), and the live sample is provably not exhaustive. Unmatched values are reported once per value via `console.warn` **and** `Sentry.captureMessage` (tagged `area: "adapter-vocabulary"`) so vocabulary drift is visible in GlitchTip, not only in logs someone happens to be tailing; `__tests__/lib/adapters/acloud-category-vocabulary.test.ts` asserts every live value is either mapped or explicitly listed in `ACLOUD_KNOWN_UNMAPPED`. Both tables are indexed via a local `safeLookup(table, key)` helper (`Object.hasOwn` guard) rather than raw bracket access — a plain object's bracket lookup inherits `Object.prototype`, so an untrusted key of `"constructor"` or `"__proto__"` would otherwise resolve truthy instead of `undefined`. RfA additionally matches single tokens (split on non-letters, checked against `FROM_RFA`'s own key order for deterministic multi-token priority) because its vocabulary is unverified — its endpoint is not generally reachable — and its unmatched fallback is `"attraction"`, not "drop"; token matching still never matches inside a word, which matters because `"bar"` is a substring of the German `"barrierefrei"`. A third stage strips a short curated list of German shop-compound suffixes (`laden`, `geschäft`, `markt`, …) and retries the bare root, to catch undelimited compounds like `"Fahrradladen"` that token-splitting alone can't reach.
...
`LocaleProvider` is nested: root layout uses `"de"` as default; `app/en/layout.tsx` wraps `/en/*` in a second `LocaleProvider initialLocale="en"`. `app/en/layout.tsx` must stay a **Server Component** so Next.js can resolve `generateMetadata` from EN SEO pages (client layouts break the metadata chain).

**Locale resolution** (`LocaleProvider`, in order): a route-supplied `initialLocale` wins outright (no detection, no storage access) → else `?lang=de|en` (and that choice is persisted) → else the stored `locale` key → else `navigator.language`'s two-letter prefix, where **only** `de` yields German and everything else falls to English. A first start with no query parameter therefore persists nothing and re-detects on every launch until the user actively picks a language. The mode (Quickstart/Expert) is irrelevant to all of this — `LocaleProvider` sits above `HomeClient` in the layout.

Both the locale and `<html lang>` are applied in a **`useLayoutEffect`, not a passive effect** (v11.0): the server can only render one language, so the real locale is always a client-side correction, and a passive effect applied it after paint — every non-German visitor saw a German first frame. Resolving during render instead is not an option (hydration mismatch, React #418); this is the same pre-paint pattern `useIsMobile` uses. `LocaleProvider` also owns `document.documentElement.lang`, since the root layout hardcodes `lang="de"` and cannot know the visitor's language — without it an English visitor on `/` got English text in a document declared German, a WCAG 2.2 SC 3.1.1 failure that makes screen readers apply German pronunciation. On `/en/*` the nested provider is the authority and the outer auto-detecting one skips the write (gated on the path, since child effects run *before* parent effects and the outer would otherwise win). The old `app/en/LangSetter.tsx` became redundant and was removed. All translations are typed via `lib/i18n/types.ts`. `distanceFromHere(m: number) => string` formats metres/km in the locale's style (DE: `"250 m entfernt"`, EN: `"250 m away"`).
...
`SettingsSheet` (`components/settings/SettingsSheet.tsx`) renders via `createPortal` and is triggered from a gear icon in `HomeClient`. It receives `settings` and `onUpdate`; `HomeClient.handleUpdateSettings` applies the patch and syncs derived state (e.g. propagates `sortOrder` → `sortBy`). Its first row is the **language switcher** (moved out of the header in v11.0 to free that slot for the mode switcher — language is a once-per-session choice, the mode is not). `LanguageSwitcher` navigates (`router.push` between `/` and `/en`), which remounts `HomeClient`, so it takes an `onBeforeNavigate` callback the sheet uses to close itself first.

### Quickstart Mode vs. Expert Mode (`settings.simpleView`)

The reduced layout (`components/simple/SimpleLayout.tsx`) is user-facing **"Quickstart-Modus"**; the full UI is **"Experten-Modus"**. The internal field stays `simpleView`.

**`simpleView` is tri-state (`boolean | null`)** — `null` = never explicitly chosen. Resolved at exactly one place, `HomeClient`'s `isQuickstart`, in this precedence: live setting → `bootSimpleView` → device fallback (`isMobile && !hasUsedAppBefore`), with `deepLinkForcesExpert` overriding all of it for a single mount. Deliberately **not** a persisted boolean default: a one-time migration that *writes* a guessed value risks permanently mis-moding an existing user, since `localStorage` is not reliably readable at cold-start layout-effect time on iOS. A `null` read is always safe to re-derive.

Three timing-critical inputs, all read in the **pre-paint layout effect** (a passive effect would paint the wrong mode first, and a lazy `useState` initialiser would be a hydration mismatch since this selects a whole different component tree):
- `bootSimpleView` — the stored choice, so the first painted frame is already right.
- `hasUsedAppBefore` — snapshotted **once per tab session** into `sessionStorage` (`ap_had_visited_at_session_start`), never re-derived per mount: `HomeClient` remounts on in-app navigation (FAQ → back), and by then this session's own search has already set `ap_visited`. `handleResetOnboarding` clears the snapshot too, so "reset to defaults" really does restore the fresh-install mode.
- `bootResolved` — gates the SEO auto-search, which must know the mode to pick Quickstart's fixed filter preset over the full UI's live filters.

**Deep links** (`docs/plans/quickstart-mode-default.md`): a link is either representable inside Quickstart or it forces Expert for that mount only (never persisted — one tapped link must not permanently eject anyone). Place links (`selectLat`/`selectLon`, incl. the combined SEO shape that also carries `q=`/`cat=`) resolve against the **raw, unfiltered** `places` state — a linked place that fails Quickstart's own toilet-category filter must still open — with a 250 m sanity cap so a search that didn't return the linked place never silently opens a different one. City+category links only map onto Quickstart's results screen for the intersection of `SEO_CATEGORY_SLUGS` and `SIMPLE_CATEGORIES` (currently cafe/restaurant/hotel/attraction); anything else keeps the Expert fallback. Any user-initiated search calls `dropQuickstartPlaceTarget()` so a link whose place never appeared can't later hijack an unrelated search. Native home-screen quick actions (parking/WC) follow whichever mode is active rather than forcing Expert: in Quickstart they run at `SIMPLE_AMENITY_RADIUS_KM` (not the full UI's persisted radius, which would also mislabel the radius in Quickstart's own results copy) and route to its amenity results screen via `initialAmenityTarget`.

**`ModeSwitcher`** (`components/ModeSwitcher.tsx`) always shows the glyph/colour of the mode a tap switches **to** (blue sparkles → Quickstart, orange graduation cap → Expert). Sparkles replaced an earlier play-circle glyph in v11.8 — on the mobile Expert header it sits directly above the map's own zoom control, where a circle-with-triangle read as a third zoom step rather than a mode switch (a compass alternative was also considered and rejected as reading like "start navigation" in this specific app). The graduation cap replaced an earlier rocket glyph in v12.14, when the mode itself was renamed Turbo → Expert — the rocket implied speed, which this mode was never about. Rendered in `MobileLayout`'s header, `SimpleLayout`'s shared `Header`, `SimpleDetail`'s own top row, and the desktop header — but deliberately **not** on Quickstart's start screen, which already carries the full "Experten-Modus an" pill (a labelled button with a subline doing exactly this), so a second bare icon there would be the same switch twice. That screen has no shared `<Header>` (no back target, no header bar) and wires its own gear, in the same rightmost position, so settings — and the language switcher inside them — stay reachable from every screen. Since the header icon alone has no visible label, the settings sheet also carries a labelled, always-shown "Quickstart-Modus" toggle row (right after language) with a one-line hint — `simple` (the prop that gates which rows the sheet shows) doubles as that toggle's current value, since it already tracks which mode the sheet was opened from.
...
- **Map locate button (v9.72):** `MapView`'s own locate button, deliberately neutral (white/grey, never blue) since it does **not** search — a tap (`HomeClient.handleLocate` → `MapView`'s `locatePanTrigger`) only pans the map and **arms** the existing "Hier suchen" pill (`onSearchHere`); a locate tap while browsing unrelated results never destroys them. `handleLocate` also reverse-geocodes the district in the background and feeds it to `ChatPanel` via `mapLocateFix`/`mapLocateFixKey`, which populates `nearbyPhase` (and therefore the location token) without running a search. Only a second tap, on the pill, actually searches — `MapView` tags each armed pill with `origin: "drag" | "locate" | "zoom"` (`searchHereOriginRef`) depending on what armed it, and `HomeClient.handleSearchHere` branches on it: `origin === "locate"` sets `chatMode("nearby")` (GPS-origin — distance shows, token stays) without exiting nearby state; a genuine drag-pan keeps the pre-existing behaviour (`chatMode("text")`, `exitNearbyTriggerKey` bump); `origin === "zoom"` (v12.10 — a same-spot radius change, e.g. zooming out via scroll/pinch/`+`/`-`/double-click with no drag) leaves `chatMode` untouched entirely, since the centre never moved. `MapViewGL`'s `moveend` handler picks the origin: the existing distance-from-`searchCenter` check wins if it trips; otherwise, if the viewport radius (`viewportRadiusKm()`) has drifted more than 40% from the currently-configured search radius, that's `"zoom"`. Full detail → **[docs/architecture/mapview-gl.md](docs/architecture/mapview-gl.md)**.
...
- `wheelmapUrl` — was populated from `accessibility.cloud`'s `infoPageUrl` when it pointed at wheelmap.org; **no source sets this anymore** since v11.10 (those records are now dropped in the adapter, see above). The field, its `merge.ts` preservation, and UI consumers are kept (degrade gracefully to always-absent) rather than removed.
...
- **Rate limits** — `/api/search`: Redis-backed (`lib/search-rate-limit.ts`, `@upstash/ratelimit` on the existing `getRedis()` client from `lib/stats.ts`), durable across cold starts and multiple instances. Per-IP: 30 searches/min general, 5/min when Google Places is requested. Plus a cross-IP circuit breaker specifically for Google Places — 15 Google-enabled searches/hour and 60/day, summed across all IPs — since a per-IP cap alone does nothing against IP rotation/botnets; sized against the Enterprise Text Search SKU (~$0.035/req, up to 9 requests/search) so a worst-case sustained attempt costs low single-digit $/hour, not unbounded. The per-IP general limiter fails open (allows the request) if Redis is unreachable; the Google limiters fail closed (skip the paid source) — availability matters more for the free path, cost control matters more for the metered one. Every Google-specific trip (per-IP, global hourly, global daily) and the no-Redis fail-closed case reports to GlitchTip (`Sentry.captureMessage`, `warning`/`info`, tagged `area: "rate-limit"`) so abuse pressure is visible/alertable rather than only inferred after a cost spike. Google Cloud's own per-API quota (Console → APIs & Services → Quotas) remains the ultimate backstop underneath all of this.
