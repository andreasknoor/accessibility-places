...
**Deep links** (`docs/plans/quickstart-mode-default.md`): a link is either representable inside Quickstart or it forces Turbo for that mount only (never persisted — one tapped link must not permanently eject anyone). Place links (`selectLat`/`selectLon`, incl. the combined SEO shape that also carries `q=`/`cat=`) resolve against the **raw, unfiltered** `places` state — a linked place that fails Quickstart's own toilet-category filter must still open — with a 250 m sanity cap so a search that didn't return the linked place never silently opens a different one. City+category links only map onto Quickstart's results screen for the intersection of `SEO_CATEGORY_SLUGS` and `SIMPLE_CATEGORIES` (currently cafe/restaurant/hotel/attraction); anything else keeps the Turbo fallback. Any user-initiated search calls `dropQuickstartPlaceTarget()` so a link whose place never appeared can't later hijack an unrelated search.
