...
Fields: `defaultSearchMode` (`"text"` | `"nearby"` | `null` = no preference; legacy `"place"` is migrated to `"text"` on load), `defaultMobileView` (`"results"` | `"map"`), `defaultChipIdx` (which chip is pre-selected, `null` = "Alle"/all categories — the app default), `sortOrder` (`"confidence"` | `"distance"`), `autoZoom` (MapView auto-fits after search), `alwaysShowParking` / `alwaysShowToilets` (passive map-layer display toggles, default `false`; persisted here, **not** in the filter-prefs key, and excluded from it in `HomeClient`), `showWeakParking` (show the weak parking tier as amber markers, incl. during a parking search; default `false`), `publicToiletsOnly` (restrict the WC layer to standalone public toilets, hiding venue WCs; default `false`), `parkingRadiusKm` (legacy radius for the amenity fetch — the amenity search now uses the main radius slider; 0.05–5.0, default 4.0).

**Critical invariant:** `SETTING_CHIPS` in `lib/settings.ts` and `CHIPS` in `ChatPanel.tsx` must stay in the **same order** — `defaultChipIdx` is an index into both simultaneously. Reordering chips in either file requires updating the other (the SEO deep-link table `SEO_CATEGORY_TO_CHIP_IDX` in `lib/cities.ts` also hard-codes these indices). The "Alle" chip in the UI is a pseudo-chip rendered after the two amenity chips and before `CHIPS` (state `selectedIdx === null` and `amenityActive == null`), **not** part of either array. The two amenity chips (`🅿`/`🚻`) are also pseudo-chips, **not** in `CHIPS`/`SETTING_CHIPS` — they are not `Category` values, so they don't affect the order invariant or `SEO_CATEGORY_TO_CHIP_IDX`. The chip strip renders in **both** text and nearby modes (always, since v8.62).
...
- `ENABLE_NEARBY_TOILETS=1` — feature flag; enables the wheelchair-WC fetch in `/api/search` (passive map layer) and `/api/nearby-parking` (the WC amenity-search chip). Independent of `ENABLE_NEARBY_PARKING`. Off by default. WCs are display-only (never enrich a place's `toilet.value`). **Both flags must be set in the Vercel production env** to be live there — they are not inferred from anything; a missing flag silently disables that layer for all users.
...
- `OVERPASS_PRIVATE_KEY` — optional; shared-secret sent as the `X-AP-Key` header **only** to the private (self-hosted) Overpass endpoint, never to public mirrors (`overpassHeaders()` in `lib/adapters/osm.ts`). Inert when unset. Used to lock the otherwise-open self-hosted server to this app — see the lockout-free rollout order in `docs/overpass-server.md`.
...
- `NEXT_PUBLIC_UMAMI_WEBSITE_ID` — optional; Umami Cloud website ID. When set, `components/UmamiScript.tsx` loads the Umami script and `lib/analytics.ts` dual-emits every `track()` event (with the `platform` dimension) to Umami in parallel with Vercel Analytics. Absent → nothing loads, Vercel stays the only sink. CSP (`next.config.ts`): `cloud.umami.is` in `script-src` (the script host) and `gateway.umami.is` in `connect-src` (Umami Cloud's data endpoint — `script.js` POSTs to `gateway.umami.is/api/send`, NOT to its own origin). Evaluation context: `docs/analytics-alternatives.md`.
- `NEXT_PUBLIC_UMAMI_SRC` — optional; overrides the Umami script URL (default `https://cloud.umami.is/script.js`) for a different region or self-hosted instance. Update the CSP if pointing at a new host.
