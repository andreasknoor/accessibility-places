...
### Quickstart Mode vs. Turbo Mode (`settings.simpleView`)

The reduced layout (`components/simple/SimpleLayout.tsx`) is user-facing **"Quickstart-Modus"**; the full UI is **"Turbo-Modus"**. The internal field stays `simpleView`.

**`simpleView` is tri-state (`boolean | null`)** — `null` = never explicitly chosen. Resolved at exactly one place, `HomeClient`'s `isQuickstart`, in this precedence: live setting → `bootSimpleView` → device fallback (`isMobile && !hasUsedAppBefore`), with `deepLinkForcesTurbo` overriding all of it for a single mount. Deliberately **not** a persisted boolean default: a one-time migration that *writes* a guessed value risks permanently mis-moding an existing user, since `localStorage` is not reliably readable at cold-start layout-effect time on iOS. A `null` read is always safe to re-derive.
...
**Deep links** (`docs/plans/quickstart-mode-default.md`): a link is either representable inside Quickstart or it forces Turbo for that mount only (never persisted — one tapped link must not permanently eject anyone). Place links (`selectLat`/`selectLon`, incl. the combined SEO shape that also carries `q=`/`cat=`) resolve against the **raw, unfiltered** `places` state — a linked place that fails Quickstart's own toilet-category filter must still open — with a 250 m sanity cap so a search that didn't return the linked place never silently opens a different one. City+category links only map onto Quickstart's results screen for the intersection of `SEO_CATEGORY_SLUGS` and `SIMPLE_CATEGORIES` (currently cafe/restaurant/hotel/attraction); anything else keeps the Turbo fallback. Any user-initiated search calls `dropQuickstartPlaceTarget()` so a link whose place never appeared can't later hijack an unrelated search. Native home-screen quick actions (parking/WC) follow whichever mode is active rather than forcing Turbo: in Quickstart they run at `SIMPLE_AMENITY_RADIUS_KM` (not the full UI's persisted radius, which would also mislabel the radius in Quickstart's own results copy) and route to its amenity results screen via `initialAmenityTarget`.

**`ModeSwitcher`** (`components/ModeSwitcher.tsx`) always shows the glyph/colour of the mode a tap switches **to** (blue sparkles → Quickstart, orange gauge → Turbo). Sparkles replaced an earlier play-circle glyph in v11.8 — on the mobile Turbo header it sits directly above the map's own zoom control, where a circle-with-triangle read as a third zoom step rather than a mode switch (a compass alternative was also considered and rejected as reading like "start navigation" in this specific app). Rendered in `MobileLayout`'s header, `SimpleLayout`'s shared `Header`, `SimpleDetail`'s own top row, and the desktop header — but deliberately **not** on Quickstart's start screen, which already carries the full "Turbo-Modus an" pill (a labelled button with a subline doing exactly this), so a second bare icon there would be the same switch twice. That screen has no shared `<Header>` (no back target, no header bar) and wires its own gear, in the same rightmost position, so settings — and the language switcher inside them — stay reachable from every screen. Since the header icon alone has no visible label, the settings sheet also carries a labelled, always-shown "Quickstart-Modus" toggle row (right after language) with a one-line hint — `simple` (the prop that gates which rows the sheet shows) doubles as that toggle's current value, since it already tracks which mode the sheet was opened from.
