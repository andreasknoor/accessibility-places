...
**Amenity focus mode "search this area"** — in focus mode (WC/parking near GPS), `MapView` shows a persistent top-centre button that re-fetches the active amenity layers at the **current map centre** with a radius derived from the visible viewport (centre→corner), via `onFocusSearchHere(center, radiusKm)` → `handleFocusSearchHere` → `fetchFocusSpotsAt`. This is deliberately separate from the venue `onSearchHere` machinery (avoids the fragile pan-detection invariant). `focusSearchCenter` (null = GPS-anchored) is threaded to MapView: when set, the focus map-fit is **skipped entirely** so the map stays exactly where the user put it (no jump to old-then-new spots). On mobile, focus mode also hides the stale "X places" pill and disables the Results/Filter tabs (exit via the always-visible focus chips).
