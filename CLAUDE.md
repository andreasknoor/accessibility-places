...
- `NEXT_PUBLIC_UMAMI_WEBSITE_ID` — optional; Umami Cloud website ID. When set, `components/UmamiScript.tsx` loads the Umami script and `lib/analytics.ts` dual-emits every `track()` event (with the `platform` dimension) to Umami in parallel with Vercel Analytics. Absent → nothing loads, Vercel stays the only sink. `cloud.umami.is` must stay in the CSP `script-src` + `connect-src` (`next.config.ts`). Evaluation context: `docs/analytics-alternatives.md`.
