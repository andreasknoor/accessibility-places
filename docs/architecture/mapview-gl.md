...
When a recentre does happen, the target does **not** land at the viewport's exact vertical centre — that only leaves half the container height above it, which a normal popup can still exceed (this was a real bug, found live: popups kept poking out above the map). Instead the marker is pushed to `RECENTER_VERTICAL_BIAS` (0.72) down the viewport via `easeTo`'s `offset` option, leaving most of the height free above it.

**`offset` sign gotcha, verified live:** the naive reading of the `.d.ts` comment ("of the target center relative to real map container center") suggests a positive `offset.y` should move the target *down* on screen. Empirically it's the opposite — a positive `offset.y` moves the target's on-screen position **up**. The current code negates the computed offset to compensate. Confirmed by clicking a marker deliberately placed right on the container's top edge and watching which direction it actually moved after recentring, twice (once wrong, once corrected) — don't re-derive this from the docs alone if touching this code, verify the direction live again.
