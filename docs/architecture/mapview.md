...
The *amenity-pan effect* (`deps: [amenityPanTrigger, mapReady]`, "zur Karte" from a parking/WC list card) has the same same-render race but a different symptom: it opens its popup synchronously, before the visibility effect's `invalidateSize()` has run, so the `popupopen` handler computes `popupMaxHeight()` off Leaflet's stale/zero cached `map.getSize()` and clamps the popup to its ~60px floor — a WC popup showing only its title, footer gone (v11.22). Fixed generally rather than by reordering the pan effects individually: `openPopupRef` tracks whichever popup is currently open (set/cleared in the `popupopen`/`popupclose` handlers), and every `invalidateSize()` call site — the visibility effect's `setTimeout` **and** the drag-resize `ResizeObserver` below — re-derives that popup's `maxHeight` via `applyFreshPopupMaxHeight()` right after. Self-healing: it doesn't matter which effect opened the popup or in what order effects ran, the correction always lands once the container is actually measured. A future third pan effect gets this for free — no per-effect coordination needed.
